...
// ---------------------------------------------------------------------------
// Comprehensive Example - PostFrame Package v1.1.0
// ---------------------------------------------------------------------------
...
class _BasicExampleState extends State<BasicExample> {
  // Controllers for the vertical and horizontal scroll regions.
  final ScrollController verticalController = ScrollController();
  final ScrollController horizontalController = ScrollController();

  // UI state: a status message + whether the automated scroll sequence finished.
  ...
  void initState() {
    ...
    // Demonstrate advanced usage: wait for multiple controllers, several
    // end-of-frame passes, and then perform auto-scroll animations.
    //
    // The callback will run AFTER:
    // * First frame render.
    // * 3 end-of-frame completions (pipeline drained each time).
    // * Controllers attached & metrics stable (up to 8 frame polls).
    PostFrame.postFrame(() async {
      setState(() => status = 'Metrics stable, starting auto-scroll...');

      // Scroll horizontally to the end.
      // Guard with hasClients to avoid exceptions if detached early.
      ...
      // Scroll vertically to roughly mid content for demonstration.
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () {
          // Re-run advanced post-frame scheduling manually to demonstrate reuse.
          setState(() {
...
class _ExtensionsExampleState extends State<ExtensionsExample> {
  ...
  void _useDebounced(BuildContext context) {
    // Call multiple times - only last executes
    for (var i = 0; i < 5; i++) {
